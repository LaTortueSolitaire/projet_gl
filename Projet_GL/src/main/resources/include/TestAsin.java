...
public class TestAsin{
  public static void main(String args[]){
    ...
    drawFunction(-1, 1, 10000);
    // drawFunction(1000000, 1000100, 10000);
    drawError(-1, 1, 100000);
    // drawError(1000000, 1000100, 100000);

     drawErrorUlp(-1, 1, 100000);
    // drawErrorUlp(1000000, 1000100, 100000);
  ...
  public static void drawFunction(float a, float b, int nbValues){
    ...
    for (int i = 0; i < data[0].length; i++) {
        ...
        data[0][i] = x;
        data[1][i] = MathForDeca.asin(x);
    }
    final ScatterPlot demo = new ScatterPlot("Custom asin", data);
    demo.show(demo);
  }
  ...
  public static void drawError(float a, float b, int nbValues){
    ...
    for (int i = 0; i < data[0].length; i++) {
        float x = ( (float) i) * step + a;
        data[0][i] = x;
        data[1][i] = Math.abs(MathForDeca.asin(x) - (float) Math.asin(x));
    }
  ...
  public static void drawErrorUlp(float a, float b, int nbValues){
    ...
    for (int i = 0; i < data[0].length; i++) {
        float x = ( (float) i) * step + a;
        data[0][i] = x;
        float trueValue = (float) Math.asin(x);
        data[1][i] = Math.abs(MathForDeca.asin(x) - trueValue)/Math.ulp(trueValue);
  ...
  public static void displaySomeValues(float[] abscisses){
    ...
    for (int i = 0; i < abscisses.length; i++){
      System.out.print((float) Math.asin(abscisses[i]));
      System.out.print('\t');
    }
    System.out.println();
    System.out.println("Values custom algorithm: ");
    for (int i = 0; i < abscisses.length; i++){
      System.out.print(MathForDeca.asin(abscisses[i]));
      System.out.print('\t');
    }
    System.out.println();
  }
