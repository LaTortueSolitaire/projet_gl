...
public class TestUlp{
  ...
  public static void drawFunction(float a, float b, int nbValues){
    ...
    for (int i = 0; i < data[0].length; i++) {
        ...
        data[0][i] = x;
        data[1][i] = MathForDeca.ulp(x);
    }
    final ScatterPlot demo = new ScatterPlot("Custom ulp", data);
    demo.show(demo);
  }
  ...
  public static void drawError(float a, float b, int nbValues){
    ...
    for (int i = 0; i < data[0].length; i++) {
        float x = ( (float) i) * step + a;
        data[0][i] = x;
        data[1][i] = Math.abs(MathForDeca.ulp(x) - (float) Math.ulp(x));
    }
  ...
  public static void drawErrorUlp(float a, float b, int nbValues){
    ...
    for (int i = 0; i < data[0].length; i++) {
        float x = ( (float) i) * step + a;
        data[0][i] = x;
        float trueValue = (float) Math.ulp(x);
        data[1][i] = Math.abs(MathForDeca.ulp(x) - trueValue)/Math.ulp(trueValue);
  ...
  public static void displaySomeValues(float[] abscisses){
    ...
    for (int i = 0; i < abscisses.length; i++){
      System.out.print((float) Math.ulp(abscisses[i]));
      System.out.print('\t');
    }
    System.out.println();
    System.out.println("Values custom algorithm: ");
    for (int i = 0; i < abscisses.length; i++){
      System.out.print(MathForDeca.ulp(abscisses[i]));
      System.out.print('\t');
    }
    System.out.println();
  }
